...
- `src/features/timeline/timeline-controller.js`: Timeline CRUD/render orchestration.
- `src/features/expenses/settlement.js`: parse participants + thuật toán settlement thuần.
- `src/app/pwa.js`: đăng ký service worker và trạng thái offline.
- `supabase/migrations/005_timeline_expense_settlement.sql`: timeline table + expense participants.
- `manifest.webmanifest`, `sw.js`: PWA app shell.
